...
# get train error
trainY_pred = trainX_principal %*% beta
train_error = log(sqrt(sum(trainY - trainY_pred)^2))

# get test error
testX_principal = predict(pca, newdata=testX)[, selected]
testY_pred = testX_principal %*% beta

test_error = log(sqrt(sum(testY - testY_pred)^2))
...
plot(x=d_vals-1,
     y=sim_output, 
     main=paste('Simulation Study: Feature Count vs. Log Cumulative Error'),
     ylab='Log Cumulative Error',
     xlab='Feature Count',
     col='blue'
     )
