...
f = as.matrix(ns(X[1, ], df=9, intercept=FALSE))
...
spline_plot = f %*% colMeans(b_cluster)

plot.new(); plot.window(xlim=c(1,2),ylim=c(1,6) )

plot(weeks, spline_plot, type = 'l')
lines(weeks, x_cluster[1, ], type = 'l')


for (series in 1:nrow(time_series)) {
  lines(weeks, time_series[series, ])
}

dim(X[clusters$cluster == 1, ])
