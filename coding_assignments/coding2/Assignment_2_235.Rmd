...
for (sim in 1:n_sims) {
  ...
  # ridge min
  default_ridge = cv.glmnet(train_x, train_y, alpha = 0)
  min_lambda = default_ridge$lambda.min
  model_ridge_min = glmnet(train_x, train_y, lambda = min_lambda)
  predictions = as.vector(predict(model_ridge_min, s = min_lambda, newx = test_x))
  results[sim, 2] = mean((test_y - predictions)^2)
  
  # lassos 
  cv.glmn
  default_lasso = cv.glmnet(train_x, train_y, alpha = 1)
  # lasso min
