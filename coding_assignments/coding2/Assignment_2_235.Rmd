...
n_sims = 5 # faster for debugging during dev
#n_sims = 50
model_names = c("Full linear model",
                "Ridge - min lambda",
                "Lasso - min lambda",
                "Lasso - 1se lambda",
                "Linear model using 1se lambda refit",
                "PCR - 10fold cv")
...
for (sim in 1:n_sims) {
  ...
  predictions = as.vector(predict(model_ridge_min, s = model_ridge$lambda.min, newx = test_x))
...
```{r part2-1b, eval = TRUE}
  ...
  stripchart(results, 
             method = "jitter", 
             main = "Simulation Study Mean Squared Prediction Errors", 
             ylabel = "Mean Squared Prediction Error")
  axis(1, at = length(model_names), labels = model_names)
  dev.off()
