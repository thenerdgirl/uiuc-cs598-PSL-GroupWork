...
for (sim in 1:n_sims) {
  ...
  id1 = sample(1:csize, n, replace=TRUE)
  id0 = sample(1:csize, n, replace=TRUE)  
  train_x = matrix(rnorm(2*n*p), 2*n, p)*s + rbind(m1[id1,], m0[id0,])
  ...
  # Allocate the n samples for class 1 to the 10 clusters
  id0 = sample(1:csize, N, replace=TRUE)  
  test_x = matrix(rnorm(2*N*p), 2*N, p)*s + rbind(m1[id1,], m0[id0,])
  ...
  # bayes 
  predicted_y_bayes = bayes_rule(test_x, s, m1, m0)
