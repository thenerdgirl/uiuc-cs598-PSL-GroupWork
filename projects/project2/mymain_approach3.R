...
##### APPROACH 3 NEW BODY #####
get_prediction = function(train, test) {
  ...
  for(dept in test_depts){
    ...
    for(store in test_stores){
      ...
      test_pred <- mycoef[1] + test_model_matrix %*% mycoef[-1]
    }
  }
...
for(fold in 1:num_folds) {
  predictions = data.frame()
  cat(">>> Reached Fold ", fold)

  file_dir = paste0('fold_', as.character(fold))
  train = read.csv(paste0('Proj2_Data/',file_dir, '/train.csv'))
  test = read.csv(paste0('Proj2_Data/',file_dir, '/test.csv')) 

  test_pred = get_prediction(train, test)
  test_pred
}
