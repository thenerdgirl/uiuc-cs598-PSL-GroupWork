...
clean_linear = function(df_in) {
  ...
  #print(df_in[ , -which(names(df_in) %in% vars_to_remove)])
  df_in = df_in[ , -which(names(df_in) %in% vars_to_remove)]
  #print("cat variable removal")
  #print(df_in)
...
View(train)
x=clean_linear(train)

View(x)

x2=clean_linear(test_x)
...
train_and_eval = function(test_x, train, test_y) {
  ...
  View(train_tree)
  View(test_x_tree)
  write.csv(test_x_tree, "C:/Users/Jargonaut/Documents/UIUC MCS-DS/CS 598 - Practical Statistical Learning/test.csv", sep=":", quote = FALSE, row.names =  FALSE, col.names =  TRUE)
