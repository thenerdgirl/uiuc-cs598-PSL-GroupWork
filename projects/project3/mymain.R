...
for (split_num in 1:5) {
  ...
  # train the model
  model = cv.glmnet(x = dtm_train, y = train$sentiment, alpha = 0, family='binomial', type.measure = c("auc"))
  ...
  # Populate mysubmission.txt
  output = data.frame(id = test$id, prob = as.numeric(paste(unlist(prediction))))
  write.table(output, file = paste0(split_dir, '/mysubmission.txt'))
