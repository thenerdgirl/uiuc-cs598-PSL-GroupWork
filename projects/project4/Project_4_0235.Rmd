...
movies = readLines(paste0('app/data/', 'movies.dat'))
...
ratings = read.csv(paste0('app/data/', 'ratings.dat'), sep = ':', colClasses = c('integer', 'NULL'), header = FALSE)
colnames(ratings) = c('UserID', 'MovieID', 'Rating', 'Timestamp')

# user data
# users = read.csv(paste0(myurl, 'users.dat?raw=true'), sep = ':', header = FALSE)
users = read.csv(paste0('app/data/', 'users.dat'), sep = ':', header = FALSE)
...
If fewer than 10 predictions are non-NA, we implemented a methodology to suggest movies sorted in alphanumeric order. So for example, if k movies are recommended, those are the top k choices and the next 10-k recommendations are the sorted movies. The reasoning for this, is that if the user hasn't sufficiently rated movies, and they aren't satisfied checking the recommendations, they are likely to want to passively browse movies. If they want to browse, an algorithm probably won't be able to help them decide. Additionally, this serves as an incentive for them to rate additional movies for better recommendations.
...
exibit_a = sim_matrix[display_movies, display_movies]
print(exibit_a)
...
new_hypothetical_user = matrix(NA, nrow = 1, ncol = ncol(sim_matrix))
names(new_hypothetical_user) = colnames(sim_matrix)
new_hypothetical_user['m1613'] = 5
new_hypothetical_user['m1755'] = 4
a_new_hypothetical_user = matrix(new_hypothetical_user, nrow = 1, ncol = ncol(sim_matrix))
...
get_recommendations(a_new_hypothetical_user)
